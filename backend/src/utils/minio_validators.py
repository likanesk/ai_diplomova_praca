...
async def check_sample_exists_for_classification(bucket_name: str, database_name: str, class_name: str, sample_name: str):
    """
    Checks if a specific sample (file) exists in a class (subfolder) within a database in the S3 bucket. (for classification).
...
async def check_sample_exists_for_regression_detection(bucket_name: str, database_name: str, sample_name: str):
    """
    Checks if a specific sample (file) exists directly within a database in the S3 bucket (for regression/detection).
    ...
    if not database_name.endswith('/'):
        database_name += '/'
    sample_path = f"{database_name}{sample_name}"

    try:
        objects = client.list_objects(bucket_name, prefix=sample_path, recursive=False)
        first_object = next(objects, None)
        if not first_object or first_object.object_name != sample_path:
            logger.error(f"Sample '{sample_name}' does not exist in database '{database_name}' in bucket '{bucket_name}'.")
            raise HTTPException(status_code=404, detail=f"Sample '{sample_name}' does not exist in database '{database_name}' in bucket '{bucket_name}'.")
    except StopIteration:
        logger.error(f"Sample '{sample_name}' does not exist in database '{database_name}' in bucket '{bucket_name}'.")
        raise HTTPException(status_code=404, detail=f"Sample '{sample_name}' does not exist in database '{database_name}' in bucket '{bucket_name}'.")
    except Exception as e:
        logger.error(f"Failed to check if sample '{sample_name}' exists in database '{database_name}' in bucket '{bucket_name}': {str(e)}")
        raise HTTPException(status_code=500, detail=f"Failed to check sample existence: {str(e)}")